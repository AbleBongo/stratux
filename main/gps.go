        ...
        if tmpSituation.GPSFixQuality >= 1 {
            ...
            if tmpSituation.GPSFixQuality >= 1 && gpsSolutionType >= 1 {
                ...
                if tmpSituation.GPSSatellites == 0 {
                    tmpSituation.GPSSatellites = 10 // Synthetic fallback satellite count
                }
                if tmpSituation.GPSHorizontalAccuracy == 0 || tmpSituation.GPSHorizontalAccuracy >= 999999 {
                    tmpSituation.GPSHorizontalAccuracy = 5.0 // Synthetic accuracy in meters
                }
                if tmpSituation.GPSVerticalAccuracy == 0 || tmpSituation.GPSVerticalAccuracy >= 999999 {
                    tmpSituation.GPSVerticalAccuracy = 8.0 // Synthetic vertical accuracy
                }
...
func isGPSValid() bool {
    isValid := false
    // Prioritize GPSFixQuality and satellite count, but allow fallback if synthetic values set
    if (stratuxClock.Since(mySituation.GPSLastFixLocalTime) < 3*time.Second) && globalStatus.GPS_connected && mySituation.GPSFixQuality > 0 {
        isValid = true
    } else if mySituation.GPSNACp >= 8 && mySituation.GPSHorizontalAccuracy <= 10.0 && mySituation.GPSSatellites >= 6 {
        // Fallback for synthetic/fallback values if GGA-only device
        isValid = true
    } else {
